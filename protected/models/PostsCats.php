 ...
 * Posts cats model
 */
class PostsCats extends CActiveRecord {
    ...
    public function relations() {
        return array(
            'posts' => array(self::HAS_MANY, 'Posts', 'catid'),
            'parent' => array(self::BELONGS_TO, 'PostsCats', 'id'),
            'childs' => array(self::HAS_MANY, 'PostsCats', 'parentid'),
            'count' => array(self::STAT, 'Posts', 'catid'),
        );
    }
...
//    }
    ...
    public function beforeSave() {
        ...
        $this->addpostperms = ( is_array($this->addpostperms) && count($this->addpostperms) ) ? implode(',', $this->addpostperms) : $this->addpostperms;
        ...
        if (count($models)) {
            foreach ($models as $model) {
                $data[$model->id] = array_merge(
                        $model->getAttributes(), array('children' => self::getChilds($model->id))
                );
            }
        }
        return $data;
    ...
    public function getChilds($id, $depth = '--') {
        ...
        if (count($childs)) {
            foreach ($childs as $model) {
                $model->title = $depth . $model->title;
                $data[$model->id] = array_merge(
                        $model->getAttributes(), array('children' => self::getChilds($model->id, $depth . $depth))
                );
            }
        }
        return $data;

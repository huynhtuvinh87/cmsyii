...
class Posts extends CActiveRecord {
    ...
    public function relations() {
        return array(
            'category' => array(self::BELONGS_TO, 'PostsCats', 'catid'),
            'author' => array(self::BELONGS_TO, 'Members', 'authorid'),
            'comments' => array(self::HAS_MANY, 'PostsComments', 'postid'),
            'lastauthor' => array(self::BELONGS_TO, 'Members', 'last_updated_author'),
            'commentscount' => array(self::STAT, 'PostsComments', 'postid'),
        );
    }
    ...
    public function getLink($name, $alias, $htmlOptions = array()) {
        return CHtml::link(CHtml::encode($name), array('/posts/view/' . $alias, 'lang' => false), $htmlOptions);
    }
    ...
    public function grabPostsByCats($cats, $limit = 10) {
        ...
        if (!Yii::app()->user->checkAccess('op_post_manage')) {
    ...
    public function beforeSave() {
        ...
        if ($this->catid) {
            $find = PostsCats::model()->findByPk($this->catid);

...
class CustomUrlManager extends CUrlManager
{
    ...
    protected function processRules()
    {
        ...
        if (($urlrules = Yii::app()->cache->get('customurlrules')) === false)
        {
            ...
            {
                $_more["<lang:({$rule['language']})>/<alias:({$rule['alias']})>"] = array('site/custompages/index');
            }
...
//            } else {
            $this->rules = array(
                //-----------------------ADMIN--------------
                "admincp" => 'admin/index/index',
                "admincp/<_c:([a-zA-z0-9-]+)>" => 'admin/<_c>/index',
                "admincp/<_c:([a-zA-z0-9-]+)>/<_a:([a-zA-z0-9-]+)>" => 'admin/<_c>/<_a>',
                "admincp/<_c:([a-zA-z0-9-]+)>/<_a:([a-zA-z0-9-]+)>//*" => 'admin/<_c>/<_a>/',
                //-----------------------ADMIN--------------
//            "<lang:({$active_lang})>/tutorials/<alias:(.*)>" => array('site/tutorials/viewcategory'),

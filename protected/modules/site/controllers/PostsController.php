 ...
 */
class PostsController extends SiteBaseController {
...
//        $this->breadcrumbs[Yii::t('tutorials', 'Tutorials')] = '';
    }
    ...
    public function actionviewcategory() {
        if (isset($_GET['alias']) && ( $model = PostsCats::model()->find('alias=:alias', array(':alias' => PostsCats::model()->getAlias($_GET['alias']))) )) {
            // Can we view it?
            $cats = PostsCats::model()->getCatsForMember();

            if (!in_array($model->id, array_keys($cats))) {
                throw new CHttpException(404, Yii::t('error', 'Sorry, We could not find that category.'));
            }

            $posts = Posts::model()->grabPostsByCats($model->id, 20);
            ...
            //$model->alias = PostsCats::model()->getAlias( $model->alias );
            // Add page breadcrumb and title
    ...
    public function actionviewposts() {
        if (isset($_GET['alias']) && ( $model = Posts::model()->with(array('category', 'author', 'comments', 'commentscount'))->find('t.alias=:alias', array(':alias' => Posts::model()->getAlias($_GET['alias']))) )) {
            // Can we view it?
            $cats = PostsCats::model()->getCatsForMember();
            ...
            if (!$model->status) {
                ...
                if (!Posts::model()->canEditTutorial($model)) {
                    throw new CHttpException(404, Yii::t('error', 'Sorry, We could not find that tutorial.'));
                }
            }
            ...
            $category = PostsCats::model()->findByPk($model->catid);
            ...
            $commentsModel = new PostsComments;
            ...
            if ($addcomments) {
                if (isset($_POST['PostsComments'])) {
                    $commentsModel->attributes = $_POST['PostsComments'];
            ...
            $totalcomments = PostsComments::model()->count($criteria);
            ...
            // Grab comments
            $comments = PostsComments::model()->orderDate()->findAll($criteria);
            ...
            Yii::app()->clientScript->registerMetaTag(Yii::app()->createAbsoluteUrl('/posts/view/' . $model->alias, array('lang' => false)), 'og:url');
            ...
            $this->breadcrumbs[$category->title] = array('/'.Yii::app()->language.'/posts/' . $category->alias, 'lang' => false);
    ...
    public function actioneditpost() {
        if (isset($_GET['id']) && ( $model = Posts::model()->findByPk($_GET['id']) )) {
            ...
            if (!Posts::model()->canEditPost($model)) {
                throw new CHttpException(403, Yii::t('error', 'Sorry, You are not allowed to perform that action.'));
            }
            ...
            if (isset($_POST['Posts'])) {
                ...
                if (!Yii::app()->user->checkAccess('op_posts_manage')) {
                    // Can we auto approve tutorials for this category?
                    $model->status = 0;
                    $cat = PostsCats::model()->findByPk($model->catid);
                    if ($cat) {
                        if ($cat->autoaddperms) {
                            $perms = explode(',', $cat->autoaddperms);
                            if (count($perms)) {
                ...
                if ($model->save()) {
                    if ($model->status) {
                        ...
                        $this->redirect(array('/posts/view/' . $model->alias, 'lang' => false));
                    } else {
    ...
    public function actionrss() {
        ...
        $criteria->order = 'postdate DESC';
        $criteria->limit = 50;
        $posts = Posts::model()->with(array('author'))->findAll($criteria);
        ...
        if ($posts) {
            foreach ($posts as $r) {
                ...
                $rows[] = array(
                    'title' => $r->title,
                    'link' => Yii::app()->createAbsoluteUrl('/posts/' . $r->alias . '-' . $r->id, array('lang' => false)),
        ...
        $data = array(
            ...
            'link' => isset($model) ? Yii::app()->createAbsoluteUrl('/posts/category/' . $model->alias, array('lang' => false)) : Yii::app()->createAbsoluteUrl('tutorials', array('lang' => false)),
            'charset' => Yii::app()->charset,
